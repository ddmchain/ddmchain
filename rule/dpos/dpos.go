...
func (c *Dpos) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	confirmRand := snap.Number;
	confirmNum := uint64(0)
	if snap.Number > 8 {
		confirmNum = snap.Number - 8
		confirmRand = uint64(1);
	}
	var confirmHeadr *types.Header
	if len(parents) > 8 {
		confirmHeadr = parents[len(parents)-9]
	} else {
		confirmHeadr = chain.GetHeaderByNumber(confirmNum);
	}

	inturn := snap.inturn(header.Number.Uint64(), signer, confirmHeadr.Hash(), confirmRand)
...
func (c *Dpos) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	confirmHeadr := chain.GetHeaderByNumber(confirmNum);
	header.Difficulty = CalcDifficulty(snap, c.signer, confirmHeadr.Hash(), confirmRand)
...
func (c *Dpos) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	if header.Difficulty.Cmp(diffNoTurn) == 0 {
		...
		delay += 2*wiggleTime + time.Duration(rand.Int63n(int64(wiggle)))
...
func (c *Dpos) CalcDifficulty(chain consensus.ChainReader, time uint64, parent *types.Header) *big.Int {
	...
	confirmHeadr := chain.GetHeaderByNumber(confirmNum);
	return CalcDifficulty(snap, c.signer, confirmHeadr.Hash(), confirmRand)
}

func CalcDifficulty(snap *Snapshot, signer common.Address, hash common.Hash, confirmRand uint64) *big.Int {
	if snap.inturn(snap.Number+1, signer, hash, confirmRand) {
