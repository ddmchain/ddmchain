...
func NewLesServer(ddm *ddm.DDMchain, config *ddm.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(ddm.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, ddm.EventMux(), ddm.Engine(), newPeerSet(), ddm.BlockChain(), ddm.TxPool(), ddm.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(ddm.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(ddm.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(ddm.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(ddm.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(ddm.ChainDb())
	return srv, nil
}
...
func newCostStats(db ddmdb.Database) *requestCostStats {
