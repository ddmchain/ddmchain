...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, ddm Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		ddm:            ddm,
		...
		chain:          ddm.BlockChain(),
		proc:           ddm.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(ddm.BlockChain(), miningLogAtDepth),
	}
	// Subscribe TxPreEvent for tx pool
	worker.txSub = ddm.TxPool().SubscribeTxPreEvent(worker.txCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = ddm.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = ddm.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
...
func (self *worker) setDDMXbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.ddm.TxPool().Pending()
	if err != nil {
