...
func newTester(t *testing.T, confOverride func(*ddm.Config)) *tester {
	...
	// Create a networkless protocol stack and start an DDMchain service within
	...
	ddmConf := &ddm.Config{
		...
		DDMXbase: common.HexToAddress(testAddress),
		DDMhash: ddmhash.Config{
			PowMode: ddmhash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(ddmConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return ddm.New(ctx, ddmConf) }); err != nil {
		t.Fatalf("failed to register DDMchain protocol: %v", err)
	}
