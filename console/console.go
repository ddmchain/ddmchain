...
import (
	...
	"github.com/ddmchain/go-ddmchain/internal/jsre"
	"github.com/ddmchain/go-ddmchain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute DDMchain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jddm", struct{}{})

	jddmObj, _ := c.jsre.Get("jddm")
	jddmObj.Object().Set("send", bridge.Send)
	jddmObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jddm);"); err != nil {
	...
	flatten := "var ddm = web3.ddm; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = ddm.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jddm.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jddm.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jddm.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jddm.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jddm.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ddm.getBalance(ddm.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gddm instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gddm metadata
	fmt.Fprintf(c.printer, "Welcome to the Gddm JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + ddm.coinbase);
		console.log("at block: " + ddm.blockNumber + " (" + new Date(1000 * ddm.getBlock(ddm.blockNumber).timestamp) + ")");
