...
import (
	...
	"github.com/ddmchain/go-ddmchain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gddm console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = ddm.getBlock(ddm.getTransaction(tx).blockHash);
  var genesis = ddm.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.ddm.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  ddm.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
