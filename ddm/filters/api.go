...
// information related to the DDMchain protocol such als blocks, transactions and logs.
...
// `ddm_getFilterChanges` polling method that is also used for log filters.
//
...
// It is part of the filter package since polling goes with ddm_getFilterChanges.
//
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ddmchain.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
//
// TODO(karalabe): Kill this in favor of ddmchain.FilterQuery.
...
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ddmchain.FilterQuery(crit), logs)
	if err != nil {
		return rpc.ID(""), err
	}
