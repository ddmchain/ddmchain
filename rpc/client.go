...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout ddm_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for DDMSubscribe requests
}
...
// DDMSubscribe registers a subscripion under the "ddm" namespace.
func (c *Client) DDMSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "ddm", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. DDMSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through DDMSubscribe.
