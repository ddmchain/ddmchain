...
func TestAccountListEmpty(t *testing.T) {
	gddm := runGddm(t, "account", "list")
	gddm.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gddm := runGddm(t, "account", "list", "--datadir", datadir)
	defer gddm.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gddm := runGddm(t, "account", "new", "--lightkdf")
	defer gddm.ExpectExit()
	gddm.Expect(`
...
`)
	gddm.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gddm := runGddm(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gddm.ExpectExit()
	gddm.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gddm.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gddm.StderrText(), m) {
...
// https://github.com/ddmchain/go-ddmchain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gddm.SetTemplateFunc("keypath", func(file string) string {
