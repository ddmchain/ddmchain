...
// gddm is the official command-line client for DDMchain.
package main
...
const (
	clientIdentifier = "gddm" // Client identifier to advertise over the network
)

var (
	// Git SHA1 commit hash of the release (set via linker flags)
	gitCommit = ""
	// DDMchain address of the Gddm release oracle.
...
// gddm is the main entry point into the system if no special subcommand is ran.
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full DDMchain node is running
