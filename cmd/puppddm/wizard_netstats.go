...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for ddmstats availability")
	if infos, err := checkDDMstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["ddmstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["ddmstats"] = infos.Report()
		ddmstats = infos.config
	}
