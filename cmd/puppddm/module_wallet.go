...
FROM puppddm/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gddm --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gddm --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ddmstats \'{{.DDMstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PuppddmNetworkID/{{.NetworkID}}/g' dist/js/ddmxwallet-master.js && \
	sed -i 's/PuppddmNetwork/{{.Network}}/g'     dist/js/ddmxwallet-master.js && \
	sed -i 's/PuppddmDenom/{{.Denom}}/g'         dist/js/ddmxwallet-master.js && \
	sed -i 's/PuppddmHost/{{.Host}}/g'           dist/js/ddmxwallet-master.js && \
	sed -i 's/PuppddmRPCPort/{{.RPCPort}}/g'     dist/js/ddmxwallet-master.js
