...
// newCppGenesisSpec converts a go-ddmchain genesis block into a Parity specific
// chain specification format.
func newCppGenesisSpec(network string, genesis *core.Genesis) (*cppGenesisSpec, error) {
	// Only ddmhash is currently supported between go-ddmchain and cpp-ddmchain
	if genesis.Config.DDMhash == nil {
	...
	spec.Params.BlockReward = (*hexutil.Big)(ddmhash.FrontierBlockReward)
...
// newParityChainSpec converts a go-ddmchain genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ddmhash is currently supported between go-ddmchain and Parity
	if genesis.Config.DDMhash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Parity's format
	...
	spec.Engine.DDMhash.Params.MinimumDifficulty = (*hexutil.Big)(params.MinimumDifficulty)
	spec.Engine.DDMhash.Params.DifficultyBoundDivisor = (*hexutil.Big)(params.DifficultyBoundDivisor)
	spec.Engine.DDMhash.Params.GasLimitBoundDivisor = (hexutil.Uint64)(params.GasLimitBoundDivisor)
	spec.Engine.DDMhash.Params.DurationLimit = (*hexutil.Big)(params.DurationLimit)
	spec.Engine.DDMhash.Params.BlockReward = (*hexutil.Big)(ddmhash.FrontierBlockReward)
	spec.Engine.DDMhash.Params.HomesteadTransition = genesis.Config.HomesteadBlock.Uint64()
	spec.Engine.DDMhash.Params.EIP150Transition = genesis.Config.EIP150Block.Uint64()
	spec.Engine.DDMhash.Params.EIP160Transition = genesis.Config.EIP155Block.Uint64()
	spec.Engine.DDMhash.Params.EIP161abcTransition = genesis.Config.EIP158Block.Uint64()
	spec.Engine.DDMhash.Params.EIP161dTransition = genesis.Config.EIP158Block.Uint64()
	spec.Engine.DDMhash.Params.EIP649Reward = (*hexutil.Big)(ddmhash.ByzantiumBlockReward)
	spec.Engine.DDMhash.Params.EIP100bTransition = genesis.Config.ByzantiumBlock.Uint64()
	spec.Engine.DDMhash.Params.EIP649Transition = genesis.Config.ByzantiumBlock.Uint64()
	...
	spec.Genesis.Seal.DDMchain.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.DDMchain.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.DDMchain.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// Python DDMchain implementation.
...
// newPyGenesisSpec converts a go-ddmchain genesis block into a Parity specific
// chain specification format.
func newPyGenesisSpec(network string, genesis *core.Genesis) (*pyGenesisSpec, error) {
	// Only ddmhash is currently supported between go-ddmchain and pyddmchain
	if genesis.Config.DDMhash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyGenesisSpec{
