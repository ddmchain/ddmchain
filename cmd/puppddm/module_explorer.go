...
FROM puppddm/explorer:latest

ADD ddmstats.json /ddmstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../ddm-net-intelligence-api && pm2 start /ddmstats.json)' >  explorer.sh && \
	echo '(cd ../ddmxchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerDDMstats is the configuration file for the ddmstats javascript client.
var explorerDDMstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.ddmchain
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.DDMstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	ddmstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerDDMstats)).Execute(ddmstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.ddmstats[:strings.Index(config.ddmstats, ":")],
		"Secret": config.ddmstats[strings.Index(config.ddmstats, ":")+1 : strings.Index(config.ddmstats, "@")],
		"Host":   config.ddmstats[strings.Index(config.ddmstats, "@")+1:],
	})
	files[filepath.Join(workdir, "ddmstats.json")] = ddmstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.ddmchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		ddmstats: infos.envvars["STATS"],
	}
	return stats, nil
}
