...
// makeWizard creates and returns a new puppddm wizard.
...
// setting up a new or managing an existing DDMchain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppddm, your DDMchain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new DDMchain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and ddmstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppddm uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering DDMchain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppddm", w.network)
