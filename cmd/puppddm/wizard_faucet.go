...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many DDMXs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.ddmstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.ddmstats = w.readString() + ":" + w.conf.ddmstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.ddmstats)
		infos.node.ddmstats = w.readDefaultString(infos.node.ddmstats) + ":" + w.conf.ddmstats
	}
