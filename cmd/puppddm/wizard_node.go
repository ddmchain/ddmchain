...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.ddmstats == "" {
		log.Error("No ddmstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.DDMhash != nil && !boot {
		fmt.Println()
		if infos.ddmhashdir == "" {
			fmt.Printf("Where should the ddmhash mining DAGs be stored on the remote machine?\n")
			infos.ddmhashdir = w.readString()
		} else {
			fmt.Printf("Where should the ddmhash mining DAGs be stored on the remote machine? (default = %s)\n", infos.ddmhashdir)
			infos.ddmhashdir = w.readDefaultString(infos.ddmhashdir)
		}
	}
	...
	if infos.ddmstats == "" {
		...
		infos.ddmstats = w.readString() + ":" + w.conf.ddmstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.ddmstats)
		infos.ddmstats = w.readDefaultString(infos.ddmstats) + ":" + w.conf.ddmstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.DDMhash != nil {
			// DDMhash based miners only need an ddmxbase to mine against
			fmt.Println()
			if infos.ddmxbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ddmxbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.ddmxbase)
				infos.ddmxbase = w.readDefaultAddress(common.HexToAddress(infos.ddmxbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy DDMchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
