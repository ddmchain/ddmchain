...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"ddmstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "ddmstats":
				if infos, err := checkDDMstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have ddmstats running, ask whether to make the secret public or not
	if w.conf.ddmstats != "" {
		fmt.Println()
		fmt.Println("Include ddmstats secret on dashboard (y/n)? (default = yes)")
