...
import (
	...
	"github.com/ddmchain/go-ddmchain/cmd/utils"
	"github.com/ddmchain/go-ddmchain/common"
	"github.com/ddmchain/go-ddmchain/console"
	"github.com/ddmchain/go-ddmchain/contracts/ens"
	"github.com/ddmchain/go-ddmchain/crypto"
	"github.com/ddmchain/go-ddmchain/ddmclient"
	"github.com/ddmchain/go-ddmchain/internal/debug"
	...
	"github.com/ddmchain/go-ddmchain/swarm"
	bzzapi "github.com/ddmchain/go-ddmchain/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the DDMchain API provider to use to settle SWAP payments",
	...
	EnsAPIFlag = cli.StringFlag{
		Name:   "ens-api",
		Usage:  "URL of the DDMchain API provider to use for ENS record lookups",
	...
	DeprecatedDDMAPIFlag = cli.StringFlag{
		Name:  "ddmapi",
...
// This init function sets defaults so cmd/swarm can run alongside gddm.
func init() {
...
var app = utils.NewApp(gitCommit, "DDMchain Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-ddmchain Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.ddmchain/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.ddmchain/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.ddmchain/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.ddmchain/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gddm only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gddm and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the ddmchain node
	...
	//register BZZ as node.Service in the ddmchain node
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := ddmclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if bzzconfig.EnsApi != "" {
			...
			ensClient = ddmclient.NewClient(client)
	...
	//register within the ddmchain node
