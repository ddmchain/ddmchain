...
func (n *Node) OpenDatabase(name string, cache, handles int) (ddmdb.Database, error) {
	if n.config.DataDir == "" {
		return ddmdb.NewMemDatabase()
	}
	return ddmdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
