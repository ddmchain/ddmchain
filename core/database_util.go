...
var (
	...
	configPrefix   = []byte("ddmchain-config-") // config prefix for the db
...
func WriteCanonicalHash(db ddmdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db ddmdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db ddmdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db ddmdb.Putter, hash common.Hash) error {
...
func WriteHeader(db ddmdb.Putter, header *types.Header) error {
...
func WriteBody(db ddmdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db ddmdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db ddmdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db ddmdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db ddmdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db ddmdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db ddmdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db ddmdb.Database) ddmdb.Database {
	return ddmdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db ddmdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db ddmdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db ddmdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
