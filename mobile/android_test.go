...
import (
	...
	"github.com/ddmchain/go-ddmchain/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gddm.LightScryptN, Gddm.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ddmchain", new NodeConfig());
			node.start();
			...
			DDMchainClient ec = node.getDDMchainClient();
	...
	// signatures too. Regression test for go-ddmchain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gddm-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ddmchain/gddmtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gddm and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ddmchain", "github.com/ddmchain/go-ddmchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gddm.aar"), "gddm.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ddmchain", "gddmtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
