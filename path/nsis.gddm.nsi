...
# 1. install gddm for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for gddm
# 5. create gddm, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable DDMCHAIN_SOCKET
...
!define GROUPNAME "DDMchain"
!define APPNAME "Gddm"
!define DESCRIPTION "Official Go implementation of the DDMchain protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since gddm is installed global
