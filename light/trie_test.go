...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb, _  = ddmdb.NewMemDatabase()
		lightdb, _ = ddmdb.NewMemDatabase()
		gspec      = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis    = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, ddmhash.NewFullFaker(), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, ddmhash.NewFaker(), fulldb, 4, testChainGen)
